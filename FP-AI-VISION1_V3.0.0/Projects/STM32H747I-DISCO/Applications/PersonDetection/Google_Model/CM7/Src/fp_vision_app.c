...
const char* output_labels[AI_NET_OUTPUT_SIZE] = {"Unknown", "Person", "Not-person"};
...
{  
  ...
  strcpy(App_Context_Ptr->Test_ContextPtr->ValidationContext.class_path,"/onboard_valid_dataset_vga_google/");
#elif CAMERA_CAPTURE_RES == QVGA_320_240_RES
  strcpy(App_Context_Ptr->Test_ContextPtr->ValidationContext.class_path,"/onboard_valid_dataset_qvga_google/");
#endif
  ...
  /*{scale,zero-point} set to {127,128} since Person Detect NN model was trained using input data normalized in the range [-1, 1]*/
  App_Context_Ptr->Ai_ContextPtr->nn_input_norm_scale=127.0f;
  App_Context_Ptr->Ai_ContextPtr->nn_input_norm_zp=128;
...
void APP_Postprocess(AppContext_TypeDef *App_Context_Ptr)
{
  ...
  ***/
  ...
  AI_Softmax(App_Context_Ptr->Ai_ContextPtr);
